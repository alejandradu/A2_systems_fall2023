...
char * Str_copy(char out[], const char src[])
{
   assert(src != NULL); 
   assert(out != NULL);
   size_t LenOut = sizeof(out), i = 0;    // sizes will include + 1 of last null
...
char * Str_concat(char out[], const char src[])
{
   assert(src != NULL);
   assert(out != NULL);
   size_t LenOut = sizeof(out), i = 0, j = 0;
...
int Str_compare(const char s1[], const char s2[])
{
   ...
   while((c1 = s1[i]) == (c2 = s2[i])) {
      i++;  
      if(i == sizeof(s1) || i == sizeof(s2)) { // is this redundant
         break;
         return 0;
...
char * Str_search(const char haystack[], const char needle[])
{
   ...
   assert(needle != NULL);
   size_t HLen = sizeof(haystack), NLen = sizeof(needle), i = 0;
   int match;
